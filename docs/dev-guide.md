...
电视**没有固定出厂 IP**，家用路由器 DHCP 池通常是 **`.100`–`.200`**（如 `192.168.0.100–200`），智能电视大多落在此区间。麻薯投屏快扫顺序：

1. 上次用过的电视 IP（若已保存）
2. 设置里手动添加的 IP / 网段前缀
3. 网关 `.1`、`.2`、`.254`
4. **`.100`–`.180`**（DHCP 核心区，含你的 `.103`）
5. 少量常见手动地址 `.10`、`.20`、`.50` 等
6. 仍未找到时，再扫该网段剩余地址

若自动扫描仍慢，在设置中填写网段 `192.168.0`，或直接 **添加 IP `192.168.0.103`** 最稳。
